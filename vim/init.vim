...
" Letter type recognized for <C-a>
set nrformats-=octal
...
" Pairs as brackets used by % command
set matchpairs+=<:>
...
function! MyTabLabel(n, is_active)
    ...
    let representing = bufname(buflist[tabpagewinnr(a:n) - 1])
    let modified = 0
    ...
    if !a:is_active
        let representing = fnamemodify(representing, ':p:t')
    endif

    return len(buflist) . (modified ? '+' : '') . ' ' . representing
endfunction
...
if dein#load_state(s:dein_cache_path)
    ...
    \ 'if': executable('opam'),
    \ 'on_ft': 'ocaml',
    \ 'hook_source': 'execute "helptags " . g:opamshare . "/merlin/vim/doc"'
    \ })
    call dein#end()
